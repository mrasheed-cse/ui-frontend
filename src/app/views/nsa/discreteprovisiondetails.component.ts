import {
  ...
  @Component({
	selector: 'app-discreteprovisiondetails',
	templateUrl: './discreteprovisiondetails.component.html',
	  styles: [],
	providers: [WorkflowsService,AppGlobals,LoginService,FileoperationService]
  })
  export class DiscreteprovisiondetailsComponent implements OnInit {
	  wr_BriefId : number;
	  hop_sequence : number;
	  wrBriefName : string;
	  userGroup_id : number;
  ...
  LoadPreviousHopsData(){
		  ...
		  this.workFlowsService.LoadPreviousHopsField(this._global.wrid_DescProvisioning,this.wr_BriefId,this.groupID,this.hop_sequence).subscribe(
			  data => { 
					  //console.log(data);
  ...
  onDoneClick(event: any){
		  ...
		  this.isLoading = true;		 
		   ...
		   if(this.hop_sequence==4) // LAST HOP IN De-PROVISION
			  this.isDone = true;
			this.workFlowsService.UpdateExistiongWorkRequest(this.workFlowsService.FormatWorkRequestNameForAPI(this.wrBriefName),this._global.wrid_DescProvisioning, this.groupID,this.userID,this.hop_sequence,"",this.isDone).subscribe(
		res  =>  {
