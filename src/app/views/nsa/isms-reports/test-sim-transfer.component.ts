...
@Component({
    selector: 'app-test-sim-transfer',
    templateUrl: './test-sim-transfer.component.html',
    styleUrls: ['./test-sim-transfer.component.scss'],
...
export class TestSimTransferComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute, private router: Router,private loginService: LoginService,private http: HttpClient, private _global: AppGlobals, private ismsreportService: IsmsreportService, private workFlowsService: WorkflowsService) {
    ...
    this.ismsreportService.TestSimActionReport("","","","","","",0,0,0,"Transfer", this.groupID).subscribe(
        data  =>  {
  ...
  this.gridColumnApi = params.columnApi;
   ...
   onSearchSubmit() {
    ...
    this.ismsreportService.TestSimActionReport(this.reqname.value,
      ...
      this.startMSISDN.value,this.endMSISDN.value,simOwner_value_asId,0,0,"Transfer", this.groupID).subscribe(
        data  =>  {
      console.log('response is : '+data);
