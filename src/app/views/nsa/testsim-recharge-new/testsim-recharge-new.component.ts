...
export class TestsimRechargeNewComponent implements OnInit {
  ...
  loadPendingList(){
    ...
    //GetPendingTaskList
    this.workFlowsService.loadMySimsWithSearch(this.userID, this.offset, this.searchOptions_msisdn, this.searchOptions_rqnNo, +(this.searchOptions_simStatus), this._global.wrid_testSimRecharge).subscribe(
        data => {
          if(data !=null){            
            console.log(data);
  ...
  recharge(){
    ...
    this.workFlowsService.doPendingSimActionExistsForRqnLineMsisdnId(this.tmp).subscribe(
      data => {
        console.log(data);
        if(data['message'] == "SUCCESS") this.router.navigate(['nsa/testsim-recharge', this.selectedIds]);
      },
    err => {
      console.error(err);
