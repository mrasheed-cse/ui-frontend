...
@Component({
  selector: 'app-testsim-transfer-new',
  templateUrl: './testsim-transfer-new.component.html',
  styleUrls: ['./testsim-transfer-new.component.scss'],
	providers: [WorkflowsService,AppGlobals,LoginService],
})
export class TestsimTransferNewComponent implements OnInit {
  ...
  constructor(private router: Router,private loginService: LoginService,private http: HttpClient, private _global: AppGlobals, private workFlowsService: WorkflowsService) {
    ...
    this.columnDefs = [
        ...
        {headerName: 'Transfer Req', field: 'simActionWorkRequestBriefName', sortable: true, filter: true, width: 200},
  ...
  transfer(){
    ...
    this.workFlowsService.doPendingSimActionExistsForRqnLineMsisdnId(this.tmp).subscribe(
      data => {
        console.log(data);
        if(data['message'] == "SUCCESS") this.router.navigate(['nsa/testsim-transfer', this.selectedIds]);
      },
    err => {
      console.error(err);
